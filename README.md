...
- `api_keys.yaml` and `api_mgmt.yaml` consist of variables that are responsible for managing your API keys.

✧ The file `api_keys.yaml` contains the complete list of your API keys that need to be added to your shared node during the deployment process using the `idena_shared.yaml` playbook;<br>
✦ The file `api_mgmt.yaml` contains two variables with keys that are meant to be added or removed after the installation of the shared node. This can be achieved using the `idena_node_mgmt.yaml` playbook with the tag `IdenaApiKeysUpdate`.
...
On the other hand, the Idena Node Management playbook (`idena_node_mgmt.yaml`) will use `api_keys_add` and `api_keys_remove` variables from the `./idena-sibling/group_vars/main/api_mgmt.yaml` file to add and remove keys on your shared node idena-node-proxy configuration:

✵ It will add new keys, remove old ones, and delete duplicates. After all of these manipulations, the `apiKeys` values will be synced with your local `./idena-sibling/group_vars/main/api_keys.yaml` file.
